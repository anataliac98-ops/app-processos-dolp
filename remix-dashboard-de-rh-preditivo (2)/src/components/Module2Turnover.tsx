...
export const Module2Turnover: React.FC<Module2Props> = ({
  ...
  // Compute monthly stats for the active year (for quick badges)
  const monthlyStatsForYear = useMemo(() => {
    const map = new Map<string, { desligamentos: number; admissoes: number }>();
    MONTHS_OPTIONS.forEach(m => {
      map.set(m.value, { desligamentos: 0, admissoes: 0 });
    });
    ...
    filtered.forEach(p => {
      if (p.dataDemissao) {
        ...
        if (parts && (selectedAno === 'Todos' || parts.year === selectedAno)) {
          const cur = map.get(parts.month) || { desligamentos: 0, admissoes: 0 };
          cur.desligamentos += 1;
    ...
    return map;
  }, [filtered, selectedAno]);

  // Filtered desligados based on the general Month / Year temporal filter
  const periodDesligados = useMemo(() => {
    return filtered.filter(p => {
      if (p.status !== 'Desligado') return false;
      if (selectedAno === 'Todos' && selectedMes === 'Todos') return true;
  ...
  }, [filtered, selectedAno, selectedMes]);

  // Filtered admissoes based on the general Month / Year temporal filter
  const periodAdmissoes = useMemo(() => {
    return filtered.filter(p => {
  ...
  // Relevant active headcount
  const totalHeadcount = useMemo(() => {
    if (selectedAno === 'Todos' && selectedMes === 'Todos') {
      return filtered.length;
    }
    const ativosCount = filtered.filter(p => p.status === 'Ativo').length;
    return ativosCount + periodDesligados.length;
  }, [filtered, selectedAno, selectedMes, periodDesligados.length]);

  const totalDesligamentos = periodDesligados.length;
  const totalAdmissoes = periodAdmissoes.length;
  const saldoPeriodo = totalAdmissoes - totalDesligamentos;

  const taxaTurnover = totalHeadcount > 0
    ? Number(((totalDesligamentos / totalHeadcount) * 100).toFixed(1))
    : 0;

  const precoces = periodDesligados.filter(p => p.faixaTempoCasa === '< 90 dias (Precoce)');
  const taxaPrecoce = totalDesligamentos > 0
    ? Math.round((precoces.length / totalDesligamentos) * 100)
    : 0;

  const tempoMedioCasaMeses = totalDesligamentos > 0
    ? Number((periodDesligados.reduce((acc, p) => acc + (p.tempoDeCasaMeses || 0), 0) / totalDesligamentos).toFixed(1))
    : 0;

  // Find Top Critical Role by number of departures in the selected period
  const cargoDesligadoMap = useMemo(() => {
    const map = new Map<string, number>();
    periodDesligados.forEach(p => {
      const cargo = p.cargo || 'Cargo Operacional';
  ...
  const topCargosEvasivos = useMemo(() => {
    return Array.from(cargoDesligadoMap.entries())
      .map(([cargo, count]) => {
        const percent = totalDesligamentos > 0 ? Number(((count / totalDesligamentos) * 100).toFixed(1)) : 0;
        return {
          cargo,
          count,
          percent,
  ...
  }, [cargoDesligadoMap, totalDesligamentos]);

  const cargoMaisCritico = topCargosEvasivos.length > 0 ? topCargosEvasivos[0].cargo : 'Nenhum Identificado';

  // Cross Admission Date with Departure Date by Month (Timeline Chart)
  const [chartType, setChartType] = useState<'line' | 'bar'>('line');
  const [timelineViewMode, setTimelineViewMode] = useState<'chart' | 'table'>('chart');
  ...
  const timelineData: TimelineDataPoint[] = useMemo(() => {
    ...
    filtered.forEach(p => {
      ...
      const ymA = parseDateParts(p.dataAdmissao);
      if (ymA) {
        const current = map.get(ymA.yearMonth) || { admissoes: 0, desligamentos: 0 };
        map.set(ymA.yearMonth, { ...current, admissoes: current.admissoes + 1 });
      }

      // 2. Saídas contabilizadas via Dt. Desligamento
      if (p.dataDemissao) {
        const ymD = parseDateParts(p.dataDemissao);
        if (ymD) {
          const current = map.get(ymD.yearMonth) || { admissoes: 0, desligamentos: 0 };
          map.set(ymD.yearMonth, { ...current, desligamentos: current.desligamentos + 1 });
        }
      }
    });
  ...
  // Filter timeline according to selectedAno
  const filteredTimelineData: TimelineDataPoint[] = useMemo(() => {
    ...
    timelineData.forEach(d => {
      if (d.monthKey.startsWith(`${selectedAno}-`)) {
  ...
  }, [timelineData, selectedAno]);

  // Aggregate Departures by Unidade / Filial based on periodDesligados
  const baseSectorData = useMemo(() => {
    const map = new Map<string, number>();
    periodDesligados.forEach(p => {
      const key = p.filial || 'Sem Unidade';
    ...
    return Array.from(map.entries())
      .map(([name, value]) => ({
        name,
        value,
        percent: totalDesligamentos > 0 ? Math.round((value / totalDesligamentos) * 100) : 0,
      }))
      .sort((a, b) => b.value - a.value);
  }, [periodDesligados, totalDesligamentos]);

  // Department Departures based on periodDesligados
  ...
  // Table rows based on periodDesligados and local table filters
  const tableDesligados = useMemo(() => {
    return periodDesligados.filter(p => {
  ...
  }, [periodDesligados, tipoFilter, faixaFilter]);
  ...
  // Helper label for the active temporal period
  ...
  return (
    <div id="modulo-turnover" className="space-y-6">
      ...
      {/* NOVO PAINEL DE CONTROLE: FILTRO GERAL TEMPORAL (ANO E MÊS) */}
      <div className={`p-5 sm:p-6 rounded-2xl border shadow-xs space-y-4 ${
        isDark ? 'bg-slate-800/40 border-slate-700/80' : 'bg-gradient-to-r from-sky-50/70 via-indigo-50/40 to-slate-50 border-sky-200/80'
      }`}>
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <div className={`w-8 h-8 rounded-xl border flex items-center justify-center ${
              isDark ? 'bg-sky-500/20 text-[#38BDF8] border-sky-500/30' : 'bg-white text-sky-700 border-sky-200 shadow-2xs'
            }`}>
            ...
            <div>
              ...
              <p className={`text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                Selecione o <strong>Ano todo</strong> ou analise <strong>mês a mês individualmente</strong> todas as métricas da aba
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 self-start md:self-auto">
            {isTemporalFiltered && (
              <button
                ...
                title="Limpar filtros e exibir histórico completo"
              >
                <X className="w-3.5 h-3.5" />
                <span>Limpar Filtro Temporal</span>
              </button>
            )}
          </div>
        </div>

        {/* Controles de Seleção (Dropdown Ano + Dropdown Mês) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-1">
          {/* Select de Ano */}
          <div className={`p-3 rounded-xl border flex flex-col gap-1.5 ${
            isDark ? 'bg-slate-900/80 border-slate-700/80' : 'bg-white border-slate-200 shadow-2xs'
          }`}>
            <label htmlFor="select-ano-geral" className={`text-[10px] font-bold uppercase tracking-wider ${
            ...
            <select
              ...
              className={`w-full font-bold text-xs bg-transparent border-0 outline-hidden cursor-pointer ${
          ...
          <div className={`p-3 rounded-xl border flex flex-col gap-1.5 ${
            ...
            <label htmlFor="select-mes-geral" className={`text-[10px] font-bold uppercase tracking-wider ${
              isDark ? 'text-slate-400' : 'text-slate-500'
            }`}>
              2. Selecionar Mês:
            </label>
            ...
            >
              ...
              {MONTHS_OPTIONS.map(m => {
                const stats = monthlyStatsForYear.get(m.value);
                const countDesl = stats ? stats.desligamentos : 0;
                return (
                  <option key={m.value} value={m.value} className={isDark ? 'bg-[#0f172a] text-white' : 'bg-white text-slate-900'}>
                    {m.value} - {m.name} {countDesl > 0 ? `(${countDesl} saídas)` : ''}
                  </option>
                );
              })}
            </select>
          ...
          {/* Badge Resumo do Período */}
          <div className={`sm:col-span-2 p-3 rounded-xl border flex items-center justify-between gap-3 ${
            isDark ? 'bg-slate-900/60 border-slate-700/60' : 'bg-white/80 border-slate-200 shadow-2xs'
          }`}>
            <div className="space-y-0.5">
              ...
              <p className={`text-xs font-bold ${isDark ? 'text-[#38BDF8]' : 'text-sky-700'}`}>
                {activePeriodLabel}
              </p>
            </div>
            <div className="flex items-center gap-2 font-mono text-xs">
              <span className={`px-2 py-1 rounded-lg font-bold ${
                ...
                {totalDesligamentos} saídas
              </span>
              <span className={`px-2 py-1 rounded-lg font-bold ${
                isDark ? 'bg-sky-500/20 text-[#38BDF8] border border-sky-500/30' : 'bg-sky-50 text-sky-700 border border-sky-200'
              }`}>
                {totalAdmissoes} entradas
              </span>
            </div>
          </div>
        </div>

        {/* Barra Rápida de Atalhos Mensais (Janeiro a Dezembro + Ano Todo) */}
            ...
            {MONTHS_OPTIONS.map(m => {
              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-medium text-xs whitespace-nowrap transition cursor-pointer shrink-0 border ${
                    isSelected
                      ? isDark
                        ? 'bg-sky-500 text-white font-bold border-sky-400 shadow-xs'
                        : 'bg-sky-600 text-white font-bold border-sky-700 shadow-xs'
                      : countDesl > 0
                        ? isDark
                          ? 'bg-slate-900/90 border-rose-500/40 text-slate-200 hover:border-rose-400'
                          : 'bg-white border-rose-200 text-slate-800 hover:border-rose-300 shadow-2xs'
                        : isDark
                          ? 'bg-slate-900/60 border-slate-800 text-slate-400 hover:text-slate-200'
                          : 'bg-white/80 border-slate-200 text-slate-600 hover:bg-slate-50'
                  }`}
                  title={`${m.name}: ${countDesl} saídas`}
                >
                  <span>{m.short}</span>
                  {countDesl > 0 && (
      ...
      {/* Top 3 KPI Metric Cards (Recalculated dynamically for the selected Month / Year) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`p-6 rounded-2xl border shadow-xs relative overflow-hidden flex flex-col justify-between ${
          ...
          <div className="mt-3 flex items-center justify-between text-xs">
            ...
            <span className={`font-medium ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
              {selectedMes !== 'Todos' || selectedAno !== 'Todos' ? activePeriodLabel : 'Quadro Completo'}
            </span>
          </div>
          <div className={`w-full h-1 rounded-full mt-2 overflow-hidden ${isDark ? 'bg-sky-500/30' : 'bg-sky-100'}`}>
        ...
        <div className={`p-6 rounded-2xl border shadow-xs relative overflow-hidden flex flex-col justify-between ${
          ...
          <div className="flex items-center justify-between mb-2">
            <span className={`text-[11px] font-bold uppercase tracking-wider ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              TOTAL DESLIGAMENTOS NO PERÍODO
            </span>
            <div className={`w-8 h-8 rounded-xl border flex items-center justify-center ${
          ...
          <div className="mt-3 flex items-center justify-between text-xs">
            ...
            <span className={`font-mono font-bold ${isDark ? 'text-[#38BDF8]' : 'text-sky-700'}`}>
              {totalAdmissoes} (Saldo: {saldoPeriodo >= 0 ? `+${saldoPeriodo}` : saldoPeriodo})
            </span>
          </div>
          <div className={`w-full h-1 rounded-full mt-2 overflow-hidden ${isDark ? 'bg-rose-500/30' : 'bg-rose-100'}`}>
            <div
              className="h-full bg-rose-500 rounded-full"
              style={{ width: `${totalHeadcount > 0 ? (totalDesligamentos / totalHeadcount) * 100 : 0}%` }}
            ></div>
          </div>
        </div>
        ...
        <div className={`p-6 rounded-2xl border shadow-xs relative overflow-hidden flex flex-col justify-between ${
          ...
          <div className="mt-3 flex items-center justify-between text-xs">
            ...
            <span className={`font-mono font-bold ${isDark ? 'text-amber-400' : 'text-amber-700'}`}>
              {tempoMedioCasaMeses} meses
            </span>
          </div>
          <div className={`w-full h-1 rounded-full mt-2 overflow-hidden ${isDark ? 'bg-orange-500/30' : 'bg-orange-100'}`}>
            <div
      ...
      <div className={`w-full p-6 sm:p-7 rounded-2xl border shadow-xs space-y-4 ${
        ...
        <div className={`flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b pb-4 ${
          ...
          {/* Controls: View Mode + Chart Type Toggle + Data Labels + Year Filter */}
          <div className="flex flex-wrap items-center gap-2 text-xs">
            {/* View Mode Toggle (Gráfico vs Tabela) */}
            <div className={`flex items-center p-0.5 rounded-xl border ${
              ...
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg font-medium transition cursor-pointer ${
              ...
              <button
                ...
                title="Visualizar em Tabela Organizada"
              >
            ...
            {timelineViewMode === 'chart' && (
              <>
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl border font-medium transition cursor-pointer ${
                    showDataLabels
                      ? isDark ? 'bg-indigo-500/20 text-indigo-300 border-indigo-500/40' : 'bg-indigo-50 text-indigo-700 border-indigo-200'
                      : isDark ? 'bg-slate-900/90 border-slate-700 text-slate-400 hover:text-white' : 'bg-white border-slate-200 text-slate-600 hover:text-slate-900 shadow-2xs'
                  }`}
                  title={showDataLabels ? 'Ocultar rótulos numéricos' : 'Exibir rótulos numéricos'}
                >
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border ${
            ...
            <span className="font-medium">Total Admissões:</span>
          ...
          <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border ${
            ...
            <span className="font-medium">Total Desligamentos:</span>
            <strong className="font-mono font-bold">
          ...
          {filteredTimelineData.length > 12 && timelineViewMode === 'chart' && (
            <div className={`ml-auto text-[11px] px-2.5 py-1 rounded-lg border ${
              isDark ? 'bg-slate-900/60 border-slate-700/60 text-slate-400' : 'bg-slate-50 border-slate-200 text-slate-500'
            }`}>
              ↔ Arraste horizontalmente para navegar por todos os meses
            </div>
          )}
        </div>
        ...
        {filteredTimelineData.length === 0 ? (
          ...
          <div className="w-full overflow-x-auto pb-2 scrollbar-thin">
            <div
              className="h-[360px] sm:h-[400px] pt-4"
              style={{
                minWidth: filteredTimelineData.length > 12 ? `${Math.max(750, filteredTimelineData.length * 52)}px` : '100%',
                width: '100%',
              }}
            >
                ...
                {chartType === 'line' ? (
                  <LineChart data={filteredTimelineData} margin={{ top: 25, right: 25, left: -10, bottom: 35 }}>
                    ...
                    <Line
                      ...
                      stroke="#0284C7"
                      strokeWidth={3}
                      dot={{ r: 4, fill: '#0284C7', stroke: isDark ? '#0f172a' : '#ffffff', strokeWidth: 2 }}
                    ...
                    <Line
                      ...
                      stroke="#E11D48"
                      strokeWidth={3}
                      dot={{ r: 4, fill: '#E11D48', stroke: isDark ? '#0f172a' : '#ffffff', strokeWidth: 2 }}
                  ...
                  <BarChart
                    ...
                    margin={{ top: 25, right: 25, left: -10, bottom: 35 }}
                    barGap={4}
                    barCategoryGap="20%"
                  >
                    <CartesianGrid strokeDasharray="3 3" stroke={isDark ? '#334155' : '#E2E8F0'} opacity={0.6} vertical={false} />
          ...
          /* Table View for Complete & Organized Breakdown */
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className={`text-[10px] uppercase font-bold tracking-wider border-b ${
                  ...
                  <tr>
                    <th className="px-6 py-3.5">Competência (Mês / Ano)</th>
                    <th className="px-6 py-3.5 text-center">Admissões (Entradas)</th>
                    <th className="px-6 py-3.5 text-center">Desligamentos (Saídas)</th>
                    <th className="px-6 py-3.5 text-center">Saldo Líquido</th>
                    <th className="px-6 py-3.5 text-right">Balanço</th>
                  </tr>
                </thead>
                <tbody className={`divide-y ${isDark ? 'divide-slate-700/50' : 'divide-slate-200'}`}>
                  {filteredTimelineData.map((row) => {
                    ...
                    return (
                      <tr key={row.monthKey} className={isDark ? 'hover:bg-slate-800/30' : 'hover:bg-slate-50'}>
                        <td className={`px-6 py-3.5 font-bold font-mono ${isDark ? 'text-white' : 'text-slate-900'}`}>
                          {row.label}
                        </td>
                        <td className={`px-6 py-3.5 text-center font-mono font-bold ${isDark ? 'text-[#38BDF8]' : 'text-sky-700'}`}>
                          {row.admissoes}
                        </td>
                        <td className={`px-6 py-3.5 text-center font-mono font-bold ${isDark ? 'text-rose-400' : 'text-rose-600'}`}>
                          {row.desligamentos}
                        </td>
                        <td className={`px-6 py-3.5 text-center font-mono font-bold ${
                          ...
                          {isPositive ? `+${row.saldo}` : row.saldo}
                        </td>
                        <td className="px-6 py-3.5 text-right">
                          <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold ${
                            ...
                            {isPositive ? 'Expansão' : isNeutral ? 'Equilíbrio' : 'Retração'}
                          </span>
                        </td>
                      </tr>
                    );
        ...
        <div className={`pt-3 border-t flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs ${
          ...
          <span className="font-mono">
            {selectedAno === 'Todos'
              ...
              : `Filtrado por Ano: ${selectedAno} (12 competências mensais)`}
          </span>
        </div>
      </div>
