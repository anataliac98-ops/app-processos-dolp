...
export function computeMacroRisk(
  ...
  // Pre-index by `${cargo}___${filial}` in linear O(V + P + Pt) passes
  const groupKeys = new Set<string>();
  const vagasByGroup = new Map<string, VagaRecord[]>();
  const pessoasByGroup = new Map<string, PessoaRecord[]>();
  const pontoByGroup = new Map<string, PontoRecord[]>();

  for (let i = 0; i < vagas.length; i++) {
    const v = vagas[i];
    const key = `${v.cargo}___${v.filial}`;
    groupKeys.add(key);
    let list = vagasByGroup.get(key);
    if (!list) {
      list = [];
      vagasByGroup.set(key, list);
    }
    list.push(v);
  }
  ...
  for (let i = 0; i < pessoas.length; i++) {
    ...
    groupKeys.add(key);
    let list = pessoasByGroup.get(key);
    if (!list) {
      list = [];
      pessoasByGroup.set(key, list);
    }
    list.push(p);
  }
  ...
  for (let i = 0; i < ponto.length; i++) {
    ...
    groupKeys.add(key);
    let list = pontoByGroup.get(key);
    if (!list) {
      list = [];
      pontoByGroup.set(key, list);
    }
    list.push(pt);
  }
  ...
  groupKeys.forEach(key => {
    const [cargo, filial] = key.split('___');
    const matchedVagas = vagasByGroup.get(key) || [];
    const matchedPessoas = pessoasByGroup.get(key) || [];
    const matchedPonto = pontoByGroup.get(key) || [];

    let vagasAbertas = 0;
    let vagasSubstituicao = 0;
    let slaSoma = 0;
    for (let i = 0; i < matchedVagas.length; i++) {
      const v = matchedVagas[i];
      if (v.status === 'Aberta' || v.status === 'Em Andamento') {
        vagasAbertas++;
        if (v.motivo === 'Substituição') vagasSubstituicao++;
      }
      slaSoma += (v.slaDias || 0);
    }
    const slaMedioVagas = matchedVagas.length > 0 ? Math.round(slaSoma / matchedVagas.length) : 0;

    let totalFaltasPonto = 0;
    let totalAtestados = 0;
    for (let i = 0; i < matchedPonto.length; i++) {
      const pt = matchedPonto[i];
      totalFaltasPonto += (pt.faltasInjustificadas || 0);
      totalAtestados += (pt.atestadosDias || 0);
    }

    let desligamentosRecentes = 0;
    let desligamentosPrecoces = 0;
    let headcountAtivo = 0;
    for (let i = 0; i < matchedPessoas.length; i++) {
      const p = matchedPessoas[i];
      if (p.status === 'Desligado') {
        desligamentosRecentes++;
        if (p.faixaTempoCasa === '< 90 dias (Precoce)') {
          desligamentosPrecoces++;
        }
      } else if (p.status === 'Ativo') {
        headcountAtivo++;
      }
    }
    if (headcountAtivo === 0) headcountAtivo = Math.max(1, matchedPonto.length);
  ...
  // Dynamic Predictive Diagnosis Text
...
export function computeIndividualRisk(
  ...
  // 1. Extract all open substitution names from Vagas into Map O(V)
  const openSubstitutions = new Map<string, VagaRecord>();
  for (let i = 0; i < vagas.length; i++) {
    const v = vagas[i];
    if ((v.status === 'Aberta' || v.status === 'Em Andamento') && v.pessoaSustituida) {
      const normName = v.pessoaSustituida.trim().toLowerCase();
      openSubstitutions.set(normName, v);
    }
  }

  // 2. Pre-index ponto records into Maps for instant O(1) lookups
  const pontoByMatricula = new Map<string, PontoRecord>();
  const pontoByName = new Map<string, PontoRecord>();
  for (let i = 0; i < ponto.length; i++) {
    const pt = ponto[i];
    if (pt.matricula) pontoByMatricula.set(pt.matricula, pt);
    if (pt.nome) pontoByName.set(pt.nome.trim().toLowerCase(), pt);
  }

  // 3. Combine active people or people present in ponto
  ...
  peopleMap.forEach(({ nome, matricula, cargo, filial, tempoDeCasaMeses }) => {
    ...
    const pt = pontoByMatricula.get(matricula) || pontoByName.get(normName);
